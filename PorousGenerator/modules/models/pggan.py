...
def __add_discriminator_block(old_model, n_filters, filter_size, n_input_layers=8):
    ...
    input_img = Input(batch_shape=input_shape, name='input_img')    # was new_img_shape
    input_c = Input(batch_shape=c_shape, name='input_c')
    ...
    for i in range(2, 5):
        current_layer = old_model.layers[i]
        if current_layer.name != 'input_img' and current_layer.name != 'input_c':
            block_old = current_layer(block_old)
    #block_old = old_model.layers[2](block_old)
    #block_old = old_model.layers[3](input_c)
